...
static void ds1302WriteByte(uint8_t data) {
  for (uint8_t i = 0; i < 8; i++) {
    HAL_GPIO_WritePin(DS1302_DAT_PORT, DS1302_DAT_PIN,
...
void ds1302SetDateTime(const ds1302Time_t *time) {
  ...
  ds1302WriteReg(DS1302_REG_SEC,
                 decToBcd(time->sec) & 0x7F); // CH=0 (오실레이터 동작)
...
void ds1302SetTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour,
...
void ds1302SetBuildTime(void) {
  ...
  ds1302SetTime((uint16_t)year, month, (uint8_t)day, (uint8_t)hour,
                (uint8_t)min, (uint8_t)sec);
}
